...
Item {
	...
	function successCallback(jsonData) {
		...
		root.latitude = jsonData.lat
		root.longitude = jsonData.lon
		...
		GridLayout {
			...
			QtControls.Label {
				...
				text: {
					if (!showFlagInCompact) {
						if (showIPInCompact)
							return "IP " + root.jsonData.query
						return "IP"
					}

					var country = root.jsonData.countryCode.toUpperCase()
					if (showIPInCompact)
						return country + " " + root.jsonData.query
					return country
				}
